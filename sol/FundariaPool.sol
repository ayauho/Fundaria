...
contract Pool is ESP1 {
    ...
    mapping(address=>bool) public kycApproveDelegated;
    ...
    constructor(address _scoinAddress, address _swapRouterAddress) {
        scoinAddress = _scoinAddress;
        Scoin = ESP1(_scoinAddress);
        swapRouter = SwapRouter(_swapRouterAddress);
        swapRouterAddress = _swapRouterAddress;
        approveKyc(address(this));
    }
    ...
    modifier ownerOrKycApproveDelegated {
        require(msg.sender==_owner || kycApproveDelegated[msg.sender], 'Not delegated to approve KYC');
        _;
    }  
    ...
    function setShares(uint8 _saleShare, uint8 _teamShare, uint8 _investmentGuidesRewardShare) internal {
    ...
    function payShares(uint8 typo, address to, uint amount) public onlyOwner {
        require(kycApprovedOf(to), 'ES1: recipient KYC not approved yet');
    ...
    event TeamSharesUnlocked(uint teamSharesUnlocked, uint256 time);
    function unlockTeamShares() public needPeriod(1,true) {
        require(kycApprovedOf(_owner), 'ES1: owner KYC not approved yet');
    ...
    function approveKyc(address to) public ownerOrKycApproveDelegated {
        _kycApproved[to] = true;
        emit KycApproved(msg.sender, to, block.timestamp);
    }

    function delegateKycApproving(address to) public onlyOwner {
        kycApproveDelegated[to] = true;   
    }
    
    function undelegateKycApproving(address to) public onlyOwner {
